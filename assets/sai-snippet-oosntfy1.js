;(() => {
  ...
  class NotifyMeInstance {
    ...
    _openModal() {
      ...
      // Focus the first input AFTER the open animation completes so the
      // focus ring doesn't flash mid-fade.
      const firstInput =
    ...
    _closeModal() {
      // Close instantly — no animation. The previous WAAPI close
      // animation had a stale-state machine that caused the second-open
      // to silently re-close. An instant close removes that whole class
      // of bug; the perceived UX impact is minimal because the modal is
      // covering content the shopper has decided to dismiss anyway.
      if (this._modalAnim) {
        this._modalAnim.cancel()
        this._modalAnim = null
      }
